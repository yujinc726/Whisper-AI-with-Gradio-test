...
def process_audio(audio_file, language, remove_repeated, merge, model_size, stable_ts):
    ...
    try:
        ...
        subtitles_path = f'download/{file_info["file_name"]}_{model_size}_word_ts.srt'
        if stable_ts:
        ...
        arranged_path = f'download/{file_info["file_name"]}_{model_size}_word_ts_arranged.srt'
...
with gr.Blocks(css=custom_css) as demo:
    ...
    submit_btn.click(
        fn=process_audio,
        inputs=[audio_input, language, remove_repeated, merge, model_size, stable_ts],
